...
/**
 * Broker class takes customer request and validates it and sends it over to
 * stock exchange
 */
		...
		Runnable
{
	...
	// Require for serialization. Ensures object deserialized and serialized are
	// the same.
	...
	public static void main(String[] args)
	{
		 ...
		 * Business server. Broker handles exceptions in creating Broker server
		 * and Exchange not connecting to Business.
		 */
	...
	/**
	 * Create broker class, point him to the exhcnage he trades on TODO use
	 * multiple exchanges
	 */
	public Broker()
	{
	...
	/**
	 * This is a seperate method than consturctor to enable sub classes to
	 * override (see MockBroker)
	 * 
	...
	@Override
	public boolean buyShares(String[] list, CORBAShareType type, int quantity,
			CORBACustomer customer)
	{
		...
		ShareList sharesToBuy = prepareTrade(
				new ArrayList(Arrays.asList(list)), backConvertShareType(type),
				quantity);
		if (sharesToBuy != null)
		{
	...
	{
		Customer customer = new Customer(c.name, c.street1, c.street2, c.city,
				c.province, c.postalCode, c.country);
		return customer;
	}

	@Override
	public boolean sellShares(String[] list, CORBAShareType type, int quantity,
			CORBACustomer customer)
	{
		ShareList sharesToSell = prepareTrade(
				new ArrayList(Arrays.asList(list)), backConvertShareType(type),
				quantity);

		if (sharesToSell != null)
		{
			ShareSalesStatusList shareSatusList = exchange.sellShares(
					sharesToSell, convertCustomer(customer));
			if (shareSatusList.getShares(convertCustomer(customer)) == null
					|| shareSatusList.getShares(convertCustomer(customer))
							.isEmpty())
			{
				return false;
			} else
	...
	/**
	 * @return list of company tickers on the stock exchange TODO multiple
	 *         exchanges
	 */
	@Override
	public String[] getTickerListing()
	{
		ArrayList<String> list = exchange.getListing();
	...
	@Override
	public boolean sellShareList(CORBAShareItem[] lstShares,
			CORBACustomer customer)
	{
		ShareList customerShares = new ShareList(new ArrayList(
				Arrays.asList(lstShares)));

		ShareSalesStatusList shareSalesStatusList = exchange.sellShares(
				customerShares, convertCustomer(customer));
	...
	/**
	 * Broker sells shares Customer Buys shares
	 *
	 * @param shareItems
	 * @param customer
	 * @return
	...
	/**
	 * Broker buys shares Customer sells shares
	 *
	 * @param tickers
	 *            arraylist that need to be bought
	 * @param type
	 *            type that the tickers belong to
	 * @param quantity
	 *            that wants to be bought
	 * @param customer
	 *            customer who made the request
	 * @return
	 */
	public boolean buyShares(ArrayList<String> tickers, ShareType type,
	 ...
	 * @param type
	 *            of stocks being traded
	 * @param quantity
	 *            amount of stocks
	 * @return a shareList used by exchange or null if validation fail
	 */
	private ShareList prepareTrade(ArrayList<String> tickers, ShareType type,
			int quantity)
	{
		// Prepare shares to action - honestly this should be done a
		// common.share at a time
		...
		for (String ticker : tickers)
		{
			...
			} else
			{
				...
				float price = 50;
				String orderNumber = "";
				sharesToAction.add(new ShareItem(orderNumber, ticker, type,
						price, quantity));
			}
		}
	...
	public void run()
	{
		startService(Config.getInstance().getAttr("brokerServerName"));
