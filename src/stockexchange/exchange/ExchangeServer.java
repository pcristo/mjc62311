...
public class ExchangeServer implements Runnable {
	...
	public void run(){
		...
		/*
		 * // Set up ORB properties Properties p = new Properties();
		 * p.put("org.omg.CORBA.ORBInitialPort",
		 * Config.getInstance().getAttr("namingServicePort"));
		 * p.put("org.omg.CORBA.ORBInitialHost",
		 * Config.getInstance().getAttr("namingServiceAddr"));
		 * 
		 * // Create a new object request broker ORB orb = ORB.init(args, p);
		 * POA rootpoa =
		 * POAHelper.narrow(orb.resolve_initial_references("RootPOA"));
		 * rootpoa.the_POAManager().activate();
		 * 
		 * // Create an Exchange book ... servant = new Exchange();
		 * servant.setORB(orb);
		 * 
		 * // get object reference from the servant org.omg.CORBA.Object ref =
		 * rootpoa.servant_to_reference(servant); iExchange href =
		 * iExchangeHelper.narrow(ref);
		 * 
		 * org.omg.CORBA.Object objRef =
		 * orb.resolve_initial_references("NameService"); NamingContextExt ncRef
		 * = NamingContextExtHelper.narrow(objRef);
		 * 
		 * NameComponent path[] = ncRef.to_name("exchange"); ncRef.rebind(path,
		 * href);
		 * 
		 * System.out.println("Exchange Server ready and waiting ...");
		 * 
		 * // wait for invocations from clients for (;;){ orb.run(); } }
		 * 
