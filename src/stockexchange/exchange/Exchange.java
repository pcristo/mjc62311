...
public class Exchange  extends ExchangeServerIFPOA implements Runnable{
	...
	private ShareItem issueSharesRequest(ShareItem sItem) {
		...
		synchronized (orderNum) {
			try {
                sharesIssued = bi.issueShares(new CORBAShareOrder(orderNum,"not applicable", sItem.getQuantity(), sItem.getUnitPrice(),
                        sItem.getUnitPrice(), convertShareType(sItem.getShareType()), sItem.getBusinessSymbol()));
