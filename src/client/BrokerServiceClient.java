...
/**
 * A simple client for stock broker client Created by sai sun on 5/25/2015.
 */
public class BrokerServiceClient
{
	...
	public BrokerInterface getBroker()
	{
		...
		try
		{
			...
			if (m_serverIF != null)
				m_serverIF._release();
			m_serverIF = BrokerInterfaceHelper.narrow(ncRef.resolve_str(Config
					.getInstance().getAttr("brokerServerName")));
		...
		try
		{
			...
			while (!terminate)
			{
				...
				case 1:
					...
					ArrayList<String> list = ServerDisplayMsgs.getTickerList();
					ret = service.sellShares(
							list.toArray(new String[list.size()]),
							convertShareType(tickerType), tickerQuantity,
				...
				case 2:
					...
					ArrayList<String> l = ServerDisplayMsgs.getTickerList();
					ret = service.buyShares(l.toArray(new String[l.size()]),
							convertShareType(tickerType), tickerQuantity,
							convertCustomer(customer));
					ServerDisplayMsgs.printResult(ret);
					System.out.println("Press enter key to continue...");
					...
					break;
				case 3:
					tickers = new ArrayList<String>(Arrays.asList(service
							.getTickerListing()));
					ServerDisplayMsgs
							.printList("Ticker Listing Info:", tickers);
			 ...
			 * Customer(1,"Gay Hazan","123 Money Ave","","Montreal"
			 * ,"Quebec","H4W 1N3", "Canada" );
			 */
		} catch (Exception e)
		{
	...
	{
		return new Customer(info.split(";;d")[1], info.split(";;d")[2],
				info.split(";;d")[3], info.split(";;d")[4],
				info.split(";;d")[5], info.split(";;d")[6],
				info.split(";;d")[7]);
	}
