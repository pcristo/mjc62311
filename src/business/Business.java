...
public class Business extends BusinessInterfacePOA implements Serializable{
	...
	public boolean issueShares(CORBAShareOrder aSO) {
		...
		// if no valid listed common.share was found, return false
		if (listedShare == null) {
			LoggerClient.log(getTicker() + " failed to issue shares of " + aSO.businessSymbol + " " + backConvertShareType(aSO.shareType) +
					": No valid share found for " + backConvertShareType(aSO.shareType) + " (order #" + aSO.orderNum + ")");
			return false;
		}
		...
		if (aSO.unitPriceOrder < listedShare.getUnitPrice()) {
			LoggerClient.log(getTicker() + " failed to issue shares of " + aSO.businessSymbol + " " + backConvertShareType(aSO.shareType) +
					": Order price " + aSO.unitPriceOrder + " is less than minimum issue price " +
		...
		if (aSO.quantity <= 0) {
			LoggerClient.log(getTicker() + " failed to issue shares of " + aSO.businessSymbol + " " + backConvertShareType(aSO.shareType) +
					": Invalid number of shares requested (order #" + aSO.orderNum + ")");
			return false;
		}
		...
		// record to local memory file
		saveRecordToList(new ShareOrder(aSO.orderNum,aSO.brokerRef,aSO.businessSymbol,backConvertShareType(aSO.shareType),aSO.unitPrice,(int)aSO.quantity,aSO.unitPriceOrder));

		// return true
		LoggerClient.log(getTicker() + " successfully issued " + aSO.quantity + " shares of " + aSO.businessSymbol +
				" " + aSO.shareType + " (order #" + aSO.orderNum + ")");
		return true;
	}
	...
	public BusinessInfo getShareInfo(CORBAShareType aShareType) {
		...
		for (Share s : sharesList)
			if (s.getShareType().equals(backConvertShareType(aShareType)))
				return new BusinessInfo(s.getBusinessSymbol(),convertShareType(s.getShareType()),s.getUnitPrice());
	...
	private BusinessInfo[] convertShareToBusinessInfo()
	{
		...
		{
			BusinessInfo info = new BusinessInfo(share.getBusinessSymbol(),convertShareType(share.getShareType()),share.getUnitPrice());
			list.add(info);
		}
